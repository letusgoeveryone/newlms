...
//            criteria.add(Restrictions.eq("teacherIdcard", userName));
    ...
    public static boolean deleteTempTeacherByUserName(String userName) {
        session = HibernateUtil.getSessionFactory().openSession();
        Transaction transaction = session.beginTransaction();
        try {
            //操作
            //Criteria criteria;
            QueryResult queryResult = getTempTeacherByUserName(userName);
...
//        deleteTempTeacherById(6);
//        deleteTempTeacherById(7);
//    }
