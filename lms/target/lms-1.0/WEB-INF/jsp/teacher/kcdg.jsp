...
<html>
    ...
    <body>  
         ...
         <span  id = "DGword"></span>  
         <div id="dgxsid" style="display: none"> <iframe id="kcdgurl" frameborder="0" scrolling="no" height="700px" width="100%" name="content" ></iframe></div>
         <div id="dgscid"><input type="file" name="uploadify" id="dgmultiple_file_upload" />
		<a class="btn btn-primary" onclick="ksscdg()">开始上传</a>
		<a class="btn btn-primary" href="javascript:$('#dgmultiple_file_upload').uploadify('cancel','*')">取消上传</a>
                <a class="btn btn-primary" href="javascript:$('#dgmultiple_file_upload').uploadify('stop','*')" hidden=true id="stopUpload">停止上传</a></div><br>
        ...
        function lookcourseDGword(){
            ...
            $.ajax({
                    ...
                    success: function (data){
                             ...
                             if(data[1]==="0"){$("#dgscid").show();}else{  $("#dgscid").hide();}
                    },
                    error: function () {
                        alert("出错了，未知原因！");
                    }
...
//            alert(coursename);
        ...
        function dgyl(url){
            $("#dgxsid").show();
            url = "http://localhost:8080/Web/getswf?uri="+url;
