    ...
    $(function () {
         $("#zymultiple_file_upload").uploadify       ({
            ...
            function hmsubmitht() {
                ...
                $.ajax({
                    url: 'homeworksubmit?scid=${scid}&homeworkid=${homeworkid}',
                    type: 'POST',
                    ...
                    success: function (returndata) {
                        if (returndata == "1") {
                            alert("\n\n作业提交成功！\n\n");
                            window.location.href = "<%=path%>/student/Course?cou=${scid}#zy";
                        } else {
                            alert("\n\n作业提交失败！\n可能原因：作业已过期不存在\n");
                        }
        ...
        <p>指导<br>
            ${Hwhelp}</p>
        <br>作业的附加资源<br>
        <ol class="breadcrumb" id="breadcour">
            ${Hwattachment}</ol>
        您可以在下面的编辑框里编辑作业内容，您所提交的作业内容将会被老师查看和批改。<br>
        <script id="myEditor" style="width: 780px;height:300px;">${HwtextWork}</script>
        <script type="text/javascript">var ue = UM.getEditor('myEditor');</script><br>
        <p>您还可以：<a class="btn btn-primary" onclick="myModalshow()">编辑附件</a></p>
        如果您确认已经完成了作业的编辑，您可以选择
        <br><a class="btn btn-primary"  onclick="hmsubmitht()"> 保存作业 </a> <a class="btn btn-primary" href="<%=path%>/student/Course?cou=${scid}#zy"> 不保存 </a>
