...
<!--        <div id="ylkenr"  style="min-height: 50px;border:0px solid #b8dcff" > <iframe id="kcnrurl" frameborder="0" scrolling="no" height="700px" width="100%" name="content" ></iframe></div>-->
        ...
        function lookmu(){
          ...
          function ckkcnr(){ 
               ...
               if(node.attributes==="2"||node.attributes==="1"){document.getElementById("kcnr").innerHTML="";return true;}
		...
		$("#multiple_file_upload").uploadify({
          ...
          function re(){
              var a,b,c;
              var node = $('#tt1').tree('getSelected');
              if(node.attributes==="1"){alert("此级节点不能删除");return true;}//1
              if(node.attributes==="2"){if(node.id===1||node.id===2||node.id===3||node.id===4){alert("此特殊节点不能删除");return true;}}//2
              if(node.id===1000||node.id===2000||node.id===3000||node.id===4000||node.id===5000||node.id===6000||node.id===7000||node.id===8000){alert("此特殊节点不能删除");return false;}         
              if(node.attributes==="2"){root=$('#tt1').tree('getParent',node.target);a=root.id;b=node.id;c="null";}
              if(node.attributes==="3"){ parent=$('#tt1').tree('getParent',node.target);root=$('#tt1').tree('getParent',parent.target);a=root.id;b=parent.id;c=node.id; }
