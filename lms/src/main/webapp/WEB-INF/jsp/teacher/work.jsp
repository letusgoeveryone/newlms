    ...
    <body>     
       ...
       <script>
		...
		$("#zymultiple_file_upload").uploadify({
            ...
            function uploadWork(){
                var miaoshu = $("#zyms").val();if(miaoshu===" "||miaoshu===""){alert("作业描述没有添加!");return true;}
                var term  = ${term}; 
                var courseName= "${courseName}";
                var time = $('#dd').datebox('getValue');
                var onetime = $('#onexgtjzyksdd').datebox('getValue');
                if(time===""||onetime===""){ alert("未设置学生作业时间!");return true;}
                var arred = [];     
                arred.push(UM.getEditor('homework').getContent());arred=arred+"";
                if(UM.getEditor('homework').hasContents()===false){alert("作业要求需要编辑哦!");return true;}
                    $.ajax({
        ...
        function tjzygg(){
                var id = workid;
                var miaoshu = $("#xgzyms").val();if(miaoshu===" "||miaoshu===""){alert("作业描述没有添加!");return true;}
                var term  = ${term}; 
                var courseName= "${courseName}";
                var time = $('#xgdd').datebox('getValue');
                var starttime = $('#xgtjzyksdd').datebox('getValue');
                if(time===""||starttime===""){ alert("未设置学生提交作业截至时间!");return true;}
                var arred = [];     
                arred.push(UM.getEditor('xghomework').getContent());arred=arred+"";
                if(UM.getEditor('xghomework').hasContents()===false){alert("作业要求需要编辑哦!");return true;}
                    $.ajax({
          ...
          function ckallclass(){
             ...
             $.ajax({
                        ...
                        success: function (data) {
                             clear();
                             if(data.length===1&&data[0]==="0"){$("#bjalllk").hide();}
                             else{
                                 $("#bjalllk").show();
          ...
          function czxs(n){
              ...
              $.ajax({
                        ...
                        success: function (data) {
                          if(data[0]!=="0"){$("#stuwork").show();document.getElementById("stutable").innerHTML=data[0];
                          document.getElementById("bgbt").innerHTML="共有"+data[data.length-1]+"位同学已经交作业";
                            }
                          else{document.getElementById("stutable").innerHTML="";document.getElementById("bgbt").innerHTML="";}
                        },
                        error: function () {
