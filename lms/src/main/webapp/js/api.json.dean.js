...
DeanAPI.setDS = {
    uInfo: function(path){
        if (path === undefined){ path = DeanAPI.Path.uInfo[0]; }
        $.ajax({
            url: path,
            type: 'get',
            async: false,
            ...
            success: function (data) {
                DeanAPI.uInfo.id = data.teacherId;
                DeanAPI.uInfo.sn = data.teacherSn;
                DeanAPI.uInfo.name = data.teacherName;
                DeanAPI.uInfo.ID = data.teacherIdcard;
                DeanAPI.uInfo.grade = data.teacherPosition;
                DeanAPI.uInfo.college = data.teacherCollege;
                DeanAPI.uInfo.tel = data.teacherTel;
                DeanAPI.uInfo.qq = data.teacherQq;
                DeanAPI.uInfo.pw = data.teacherPwd;
                DeanAPI.uInfo.sex = data.teacherSex;
                DeanAPI.uInfo.position = data.teacherPosition;
                DeanAPI.uInfo.enrolling = data.teacherEnrolling;
                DeanAPI.uInfo.roleValue = data.teacherRoleValue;
                DeanAPI.uInfo.termCourse = data.termCourse;
            },
            error: function () {
                alert("数据 [个人信息] 传输失败 ！");
            }
 ...
 * 
 * @param {String} param
 * @param {String | DeanAPI.Path} path
 * @returns {Number}
 */
DeanAPI.updatePersonalInfo = function(param,path){
    var status = 0;
    if (path === undefined ? true : false) {
        path = DeanAPI.Path.uInfo[1];
    };
    $.ajax({
        url: path + param,
        type: 'post',
...
DeanAPI.updatePassword = function(op, np, path){
    var status = 0;
    if (path === undefined ? true : false) {
        path = DeanAPI.Path.uInfo[2];
    };
    $.ajax({
...
function updatePersonalInfo() {
    ...
    var status = DeanAPI.updatePersonalInfo(param);
    if (status === 0) {
        $('#snackbar').snackbar({
            alive: 10000,
...
function updatePassword() {
    ...
    if (checkPassword()) {
        var status = DeanAPI.updatePassword(hex_md5($("#op").val()), hex_md5($("#np").val()));
...
// 初始化
var uInfo = DeanAPI.uInfo;
var UProfile;
bindInfo(UProfile, 'uinfo');
if (DeanAPI.uInfo.sex === true) {
